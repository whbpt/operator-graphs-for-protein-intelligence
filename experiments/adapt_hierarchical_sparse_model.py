...
from experiments.train_hierarchical_candidate_router import (
    evaluate,
    summarize,
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--hierarchy-weight", type=float, default=0.2)
...
def sparse_objective(
    ...
    hierarchy_loss = 0.5 * (
        router.hierarchical_kl_loss(
            output["encoded_task"], query_positions, dense_target  # type: ignore[arg-type]
        )
        + router.hierarchical_kl_loss(
            output["encoded_task"],  # type: ignore[arg-type]
            query_positions,
    ...
    return loss, {
        ...
        "hierarchy_loss": float(hierarchy_loss.detach().cpu()),
...
def main() -> None:
    ...
    families = pd.read_csv(path_args.representations / "families.csv")
    ...
    candidate_budget = args.beam_size * int(router_config["leaf_size"])
    router = HierarchicalSegmentRouter(
