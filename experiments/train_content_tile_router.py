...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--tile-dim", type=int, default=16)
    parser.add_argument("--tiles", type=int, default=12)
    parser.add_argument("--selected-tiles", type=int, default=5)
...
def evaluate(
    ...
    for example in examples:
        ...
        dense_message = interaction_message(
            model, output, tokens, target_position, dense.indices, dense.values
        )
        ...
        work_ratio = (
            2 * router.tiles * router.tile_dim + evaluated_pairs * exact_dim
        ) / (valid_pairs * exact_dim)
        assignment_counts = torch.bincount(
            routed.hard_assignments[0], minlength=router.tiles
        ).float()
        rows.append(
            {
                ...
                "candidate_teacher_recall": overlap_recall(candidates, strong_positions),
                "neighbor_teacher_recall": overlap_recall(neighbors, strong_positions),
                "evaluated_pairs": evaluated_pairs,
                "evaluated_tiles": int(routed.evaluated_tiles[0, 0]),
                "valid_pairs": valid_pairs,
                "pair_fraction": evaluated_pairs / valid_pairs,
                "work_ratio": work_ratio,
                "tile_capacity_max": float(assignment_counts.max()),
                "tile_capacity_min": float(assignment_counts.min()),
...
def main() -> None:
    ...
    router = ContentTileRouter(
        ...
        tiles=args.tiles,
        selected_tiles=args.selected_tiles,
